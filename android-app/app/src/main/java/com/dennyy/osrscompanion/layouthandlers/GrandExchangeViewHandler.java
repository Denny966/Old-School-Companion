...
public class GrandExchangeViewHandler extends BaseViewHandler implements View.OnClickListener {
    ...
    public GrandExchangeViewHandler(Context context, View view) {
        super(context, view);
    ...
    public void handleGeData(JsonItem jsonItem, String result) {
        try {
    ...
    public boolean allowUpdateItem() {
        ...
        if (adapter.getItem(selectedAdapterIndex).id.equals("-1")) {
            refreshLayout.setRefreshing(false);
            return false;
        }
        return true;
        ...
        Utils.getString(Constants.GE_UPDATE_URL, GEUPDATE_REQUEST_TAG, new Utils.VolleyCallback() {
            ...
            public void onError(VolleyError error) {
                if (error instanceof TimeoutError || error instanceof NoConnectionError) {
                    ...
                    handleGeUpdateData(cachedData.data);
                }
                else {
    ...
    private void loadGraph(final String id) {
    ...
    public void handleGeGraphData(String result) {
        try {
            LineChart chart = (LineChart) view.findViewById(R.id.ge_item_graph);
            JSONObject dailyGraphData = new JSONObject(result).getJSONObject("daily");
            ...
            JSONObject trendGraphData = new JSONObject(result).getJSONObject("average");
    ...
    public boolean wasRequesting() {
        return wasRequestingGe || wasRequestingGegraph || wasRequestingGeupdate;
    }
